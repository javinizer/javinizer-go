...
<div class="space-y-4 lg:sticky lg:top-6 lg:self-start lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto">
	{#if showPosterPanel}
		<Card class="p-4">
			<div class="flex items-center justify-between gap-2 mb-3">
				<h3 class="font-semibold text-sm">{currentMovie.should_crop_poster ? m.review_poster_cropped() : m.review_poster()}</h3>
				<div class="flex gap-1">
					{#if onResetPoster}
						...
						>
							{#snippet children()}<RotateCcw class="h-3 w-3 mr-1" />{m.review_reset()}{/snippet}
						</Button>
					{/if}
					<Button
						size="sm"
						...
						class="text-xs"
					>
						{#snippet children()}{m.review_adjust_crop()}{/snippet}
					</Button>
				</div>
			</div>
			{#if displayPosterUrl}
	...
	{#if showCoverPanel}
		<Card class="p-4">
			<div class="flex items-center justify-between gap-2 mb-3">
				<h3 class="font-semibold text-sm">{m.review_cover_fanart()}</h3>
				{#if onResetCover}
					<Button
						size="sm"
						variant="outline"
	...
	{#if showTrailerPanel && currentMovie.trailer_url}
		<Card class="p-4">
			<h3 class="font-semibold mb-3 text-sm">{m.review_trailer()}</h3>
			<Button class="w-full" onclick={() => (showTrailerModal = true)}>
				{#snippet children()}
					<Play class="h-4 w-4 mr-2" />
					{m.review_play_trailer()}
				{/snippet}
			</Button>
		</Card>
	{/if}
	...
	{#if showScreenshotsPanel && currentMovie.screenshot_urls && currentMovie.screenshot_urls.length > 0}
		<Card class="p-4">
			<h3 class="font-semibold mb-3 text-sm">{m.review_screenshots_count({ count: currentMovie.screenshot_urls.length })}</h3>
				...
				{#each (showAllSidebarScreenshots ? currentMovie.screenshot_urls : currentMovie.screenshot_urls.slice(0, 4)) as url, index}
					<div class="relative group">
						...
						<button
							...
							use:tooltip={m.review_use_as_poster()}
						...
						<button
							...
							use:tooltip={m.review_use_as_cover()}
