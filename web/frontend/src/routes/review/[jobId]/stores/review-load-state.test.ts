...
describe('deriveReviewLoadPhase', () => {
	it('loads while the initial request or retry is fetching without a job', () => {
		expect(deriveReviewLoadPhase({ hasUsableJob: false, isPending: true, isFetching: true, hasError: false })).toBe('loading');
		expect(deriveReviewLoadPhase({ hasUsableJob: false, isPending: false, isFetching: true, hasError: true })).toBe('loading');
	});

	it('shows a blocking error after an initial request settles with no job', () => {
		expect(deriveReviewLoadPhase({ hasUsableJob: false, isPending: false, isFetching: false, hasError: true })).toBe('error');
	});

	it('keeps usable content with a refresh error while fetching or settled', () => {
		expect(deriveReviewLoadPhase({ hasUsableJob: true, isPending: false, isFetching: true, hasError: true })).toBe('refresh-error');
		expect(deriveReviewLoadPhase({ hasUsableJob: true, isPending: false, isFetching: false, hasError: true })).toBe('refresh-error');
	});

	it('returns ready for successful usable data', () => {
		expect(deriveReviewLoadPhase({ hasUsableJob: true, isPending: false, isFetching: false, hasError: false })).toBe('ready');
	});
