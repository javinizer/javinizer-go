...
describe('apply plan', () => {
	it.each([
		...
		expect(normalizeApplyPlan(plan).destination).toBe(operation === 'organize' ? '/dest' : undefined);
	});
	...
	it('rejects unsupported runtime versions, enums, and replacement combinations', () => {
		...
		const invalidEnum = { ...defaultApplyPlan('organize', '/dest'), media_policy: 'unknown' } as never;
	...
	it('maps presets and clears provenance after custom edits', () => {
		...
		expect(preset.merge).toEqual({ scalar_strategy: 'prefer-scraper', array_strategy: 'replace', source_preset: 'aggressive' });
