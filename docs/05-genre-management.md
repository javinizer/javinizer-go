...
> **Note:** Genre replacement is distinct from word replacement. Word replacements (`metadata.word_replacement`, disabled by default) replace text across all text fields **and each genre token**, and run *before* genre replacement. Entries have a `match_mode`: `literal` (default) or `wildcard`. Literal entries without `*` match as plain substrings; literal entries containing `*` (the censor character) match as whole tokens; wildcard entries treat `?`/`？` as a one-or-more run of the censor-glyph class (`*`, `＊`, `○`, `◯`, `〇`, `●`, `×`, `✕`, `✖`), bounded as whole tokens like `*` entries (#228). Everything else in a wildcard pattern is literal — no regex/class syntax. Note a wildcard entry with no sentinel matches token-bounded (not substring), so flipping a literal entry to wildcard can TIGHTEN matching — a candidate match is rejected when immediately preceded or followed by `*` or a Latin-script letter (#106/#227) — see the [`word` command](./03-cli-reference.md#word) and [Metadata Management Defaults](./02-configuration.md#metadata-management-defaults). See [How It Works](#how-it-works) for the full pipeline.
