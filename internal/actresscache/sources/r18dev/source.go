...
func (s *source) Collect(ctx context.Context, options actresscache.SourceOptions, emit func(actresscache.Candidate) error) error {
	path := strings.TrimSpace(options.R18DevDumpPath)
	if path == "" && options.Parameters != nil {
		path = strings.TrimSpace(options.Parameters["r18dev.dump"])
	}
	if path == "" && options.Parameters != nil {
		path = strings.TrimSpace(options.Parameters["dump"])
	}
	if path == "" {
		return fmt.Errorf("r18dev source requires --r18dev-dump or --option r18dev.dump=PATH")
	}
	store, err := r18devdump.Open(path)
	if err != nil {
		return fmt.Errorf("open r18.dev dump: %w", err)
	}
	defer func() { _ = store.Close() }()
	actresses, err := store.ListActresses(ctx)
	if err != nil {
		return err
	}
