...
func TestCollectReadsActressesFromDump(t *testing.T) {
	...
	err = listerSource(t, path).Collect(context.Background(), actresscache.SourceOptions{}, func(candidate actresscache.Candidate) error {
...
func TestCollectRequiresDumpPath(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "requires a dump lister")
}
...
`
	...
	require.NoError(t, err)
	count := 0
	err = listerSource(t, path).Collect(context.Background(), actresscache.SourceOptions{Limit: 1}, func(actresscache.Candidate) error { count++; return nil })
	require.NoError(t, err)
...
`
	...
	var mu sync.Mutex
	active, maximum := 0, 0
	err = listerSource(t, path).Collect(context.Background(), actresscache.SourceOptions{Workers: 4}, func(actresscache.Candidate) error {
		mu.Lock()
		active++
		if active > maximum {
			maximum = active
