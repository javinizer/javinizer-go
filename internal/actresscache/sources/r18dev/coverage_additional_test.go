...
// listerSource returns a dump-backed source for tests, owning the store's
// lifetime inside the test.
func listerSource(t *testing.T, path string) actresscache.Source {
	t.Helper()
	store, err := r18devdump.Open(path)
	require.NoError(t, err)
	t.Cleanup(func() { _ = store.Close() })
	return NewFromLister(func(ctx context.Context, _ int) ([]models.DumpActress, error) {
		return store.ListActresses(ctx)
	})
}
...
func TestCollectSupportsGenericDumpParameterAndLifecycleCallbacks(t *testing.T) {
	...
	err := listerSource(t, path).Collect(context.Background(), actresscache.SourceOptions{
...
func TestCollectReturnsListerErrors(t *testing.T) {
	t.Run("missing lister", func(t *testing.T) {
		...
		require.ErrorContains(t, err, "requires a dump lister")
	})
	t.Run("query", func(t *testing.T) {
		path := filepath.Join(t.TempDir(), "empty.db")
		require.NoError(t, os.WriteFile(path, nil, 0o644))
		err := listerSource(t, path).Collect(context.Background(), actresscache.SourceOptions{}, func(actresscache.Candidate) error { return nil })
...
func TestCollectReturnsEmitterError(t *testing.T) {
	...
	err := listerSource(t, path).Collect(context.Background(), actresscache.SourceOptions{Workers: 1}, func(actresscache.Candidate) error { return want })
...
func TestCollectSkipsRowsWithoutIDs(t *testing.T) {
	...
	count := 0
	err = listerSource(t, path).Collect(context.Background(), actresscache.SourceOptions{}, func(actresscache.Candidate) error { count++; return nil })
	require.NoError(t, err)
	assert.Zero(t, count)
}
...
func TestCollectPropagatesCanceledListContext(t *testing.T) {
	...
	cancel()
	err := listerSource(t, path).Collect(ctx, actresscache.SourceOptions{}, func(actresscache.Candidate) error { return nil })
	require.Error(t, err)
}
