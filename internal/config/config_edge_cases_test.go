...
`
	...
	assert.True(t, nfo.Feature.Enabled)
	assert.Equal(t, "<ORIGINAL_TITLE>", nfo.Format.DisplayTitle)
	assert.Equal(t, "<ID>-<YEAR>.nfo", nfo.Format.FilenameTemplate)
	assert.False(t, nfo.Format.FirstNameOrder)
	assert.True(t, nfo.Format.ActressLanguageJA)
	assert.True(t, nfo.Feature.PerFile)
	assert.Equal(t, "N/A", nfo.Format.UnknownActressText)
	...
	assert.True(t, nfo.Feature.IncludeTrailer)
	assert.Equal(t, "imdb", nfo.Format.RatingSource)
	assert.Len(t, nfo.Extra.Tag, 2)
	assert.Contains(t, nfo.Extra.Tag, "JAV")
	assert.Contains(t, nfo.Extra.Tag, "Asian")
	assert.Equal(t, "Amazing content", nfo.Format.Tagline)
	assert.Len(t, nfo.Extra.Credits, 2)
	assert.Contains(t, nfo.Extra.Credits, "Scraped by Javinizer")
}
...
`
	...
	output := cfg.Output
	assert.Equal(t, "<ID> - <TITLE>", output.Template.FolderFormat)
	assert.Equal(t, "<ID> [<STUDIO>]", output.Template.FileFormat)
	assert.Len(t, output.Template.SubfolderFormat, 2)
	assert.Contains(t, output.Template.SubfolderFormat, "<YEAR>")
	assert.Contains(t, output.Template.SubfolderFormat, "<MAKER>")
	assert.Equal(t, " | ", output.Template.ActressDelimiter)
	assert.Equal(t, 150, output.Template.MaxTitleLength)
	assert.Equal(t, 200, output.Template.MaxPathLength)
	assert.True(t, output.Operation.MoveSubtitles)
	assert.Len(t, output.Operation.SubtitleExtensions, 3)
	assert.False(t, output.Operation.RenameFile)
	assert.True(t, output.Operation.GroupActress)
	assert.Equal(t, "poster-<ID>.jpg", output.MediaFormat.PosterFormat)
	assert.Equal(t, "fanart-<ID>.jpg", output.MediaFormat.FanartFormat)
	assert.Equal(t, "trailer-<ID>.mp4", output.MediaFormat.TrailerFormat)
	assert.Equal(t, "screenshot", output.MediaFormat.ScreenshotFormat)
	assert.Equal(t, "screenshots", output.MediaFormat.ScreenshotFolder)
	assert.Equal(t, 2, output.MediaFormat.ScreenshotPadding)
	assert.Equal(t, "actresses", output.MediaFormat.ActressFolder)
	assert.False(t, output.Download.DownloadCover)
	assert.False(t, output.Download.DownloadPoster)
	assert.True(t, output.Download.DownloadExtrafanart)
	assert.True(t, output.Download.DownloadTrailer)
	assert.True(t, output.Download.DownloadActress)
	assert.Equal(t, 120, output.Download.DownloadTimeout)
}
...
`
	...
	assert.Equal(t, 15, dmmSettings.PlaceholderThresholdKB)
}
