...
func publishStagedBoundOS(p StagedPublish) error {
	...
	case lerr == nil && p.NoReplace:
		...
		case oerr == nil:
			return fmt.Errorf("%w at %s — post-publish occupant preserved: %w", ErrPublishStagedForeignOccupant, p.Dest, ErrPublishStagedIdentityBreak)
		...
		default:
			return fmt.Errorf("publish of %s installed a substituted name (occupant vanished pre-binding): %w", p.Staged, ErrPublishStagedIdentityBreak)
		}
	default:
