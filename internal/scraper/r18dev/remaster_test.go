...
func TestRemaster_RentalSuffixNormalized(t *testing.T) {
	...
	assert.Equal(t, "h_003abc00123", stripRentalSuffixMarkerAware("h_003abc00123r"))

	m, series := classifyRemaster("h_003abc00123hdr")
	assert.Equal(t, "h", m)
	assert.Equal(t, "abc", series)
	m, series = classifyRemaster("n_600abc00123air")
	assert.Equal(t, "ai", m)
	assert.Equal(t, "abc", series)
