...
func TestValidateRemoteImageWithSafeClientHonorsRedirectPolicy(t *testing.T) {
	...
	client := &http.Client{
		Transport:     &http.Transport{DialContext: respondWith("HTTP/1.1 302 Found\r\nLocation: http://93.184.216.34/image.png\r\nContent-Length: 0\r\n\r\n")},
		CheckRedirect: func(*http.Request, []*http.Request) error { return policyErr },
	}
	err := ValidateRemoteImageWithSafeClient(t.Context(), client, "http://1.1.1.1/start", "agent", "")
...
func TestValidateRemoteImageWithSafeClientWrapsHTTPTransportAndLimitsRedirects(t *testing.T) {
	...
	dial := func(ctx context.Context, network, addr string) (net.Conn, error) {
		redirects++
		location := fmt.Sprintf("http://1.1.1.1/image/%d", redirects)
		return respondWith("HTTP/1.1 302 Found\r\nLocation: "+location+"\r\nContent-Length: 0\r\n\r\n")(ctx, network, addr)
	}
	client := &http.Client{Transport: &http.Transport{DialContext: dial}}
	err = ValidateRemoteImageWithSafeClient(t.Context(), client, "http://1.1.1.1/start", "", "")
...
func TestValidateRemoteImageWithSafeClientPinsProxyTarget(t *testing.T) {
	...
	// Fresh instance: the direct clone is cached per-transport, so a copy
	// would inherit the first call's transport.
	privateSocks := &pinnedProxyTransport{
		base: socks.base,
		lookup: func(context.Context, string) ([]net.IPAddr, error) {
