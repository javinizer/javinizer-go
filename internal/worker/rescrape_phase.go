...
// canonical pair legs (generation succeeded, no pre-existing bytes, no other
// family row owns the ID). Cleanup deletes them only then — never the bytes
// of a concurrent winner or an untouched older state (audit R1).
func rescrapeOwnsPosterLegs(inputs rescrapePhaseInputs, lookup *resultstore.FileLookupResult, scope *rescrapeGenScope, mr *resultstore.MovieResult, movieID string) bool {
	if mr == nil || movieID == "" || !mr.PosterGenerated || mr.PosterError != nil || scope == nil || scope.preExistedPair {
		return false
	}
	if inputs.ResultMap != nil && lookup != nil && lookup.FilePath != "" && inputs.ResultMap.OtherResultUsesMovieID(lookup.FilePath, movieID) {
		return false
	}
	return true
}
...
func withRescrapeStatus(lc rescrapeLifecycle, fn func(scope *rescrapeGenScope) (*RescrapeResult, *resultstore.MovieResult, error)) (*RescrapeResult, error) {
	...
	if err != nil {
		...
		if mv := cleanupMovie(); mv != nil && !errors.As(err, &cfe) && rescrapeOwnsPosterLegs(lc.inputs, lc.lookup, scope, movieResult, mv.ID) {
			CleanupMoviePosters(lc.inputs.Fs, lc.inputs.TempDir, lc.inputs.JobID, mv)
		}
	...
	case models.RescrapeStatusConflict:
		// audit R1: on CAS conflict the canonical pair belongs to whoever won
		// (or to the pre-op state) — delete ONLY legs this rescrape created.
		if mv := cleanupMovie(); mv != nil && rescrapeOwnsPosterLegs(lc.inputs, lc.lookup, scope, movieResult, mv.ID) {
			CleanupMoviePosters(lc.inputs.Fs, lc.inputs.TempDir, lc.inputs.JobID, mv)
		}
		if lc.inputs.HistoryRepo != nil {
			movieID := lc.lookup.OldMovieID
...
func (p *rescrapePhase) Rescrape(ctx context.Context, inputs rescrapePhaseInputs, cmd RescrapeCmd) (*RescrapeResult, error) {
	...
	outcome, err := withRescrapeStatus(lc, func(scope *rescrapeGenScope) (*RescrapeResult, *resultstore.MovieResult, error) {
		...
		// audit R1: record byte-ownership BEFORE generating — cleanup may only
		// delete what this op created.
		if movieResult.Movie != nil && inputs.Fs != nil && inputs.TempDir != "" {
			pdir := filepath.Join(inputs.TempDir, "posters", inputs.JobID.String())
			_, fe := inputs.Fs.Stat(filepath.Join(pdir, movieResult.Movie.ID+"-full.jpg"))
			_, ce := inputs.Fs.Stat(filepath.Join(pdir, movieResult.Movie.ID+".jpg"))
			if fe == nil || ce == nil {
