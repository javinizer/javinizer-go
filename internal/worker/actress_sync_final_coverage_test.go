...
func TestActressSyncManagerCreateJobFinalBranches(t *testing.T) {
	t.Run("request validation", func(t *testing.T) {
		...
		_, err := manager.CreateJob(context.Background(), ActressSyncCreateRequest{Scope: "invalid"})
...
func TestActressSyncManagerHeartbeatStopsOnDoneAndContext(t *testing.T) {
	...
	close(done)
	manager.heartbeat(context.Background(), "id", "token", time.Second, done)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	manager.heartbeat(ctx, "id", "token", time.Second, make(chan struct{}))
}
