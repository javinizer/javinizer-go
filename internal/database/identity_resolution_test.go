...
func TestIdentityResolutionAliasAndDMMHierarchy(t *testing.T) {
	...
	require.Error(t, err)
	_, err = findCandidateByNameEvidenceTx(db.DB, &models.Actress{})
	require.Error(t, err)
...
func TestIdentityResolutionDatabaseErrors(t *testing.T) {
	...
	require.Error(t, err)
	_, err = findCandidateByNameEvidenceTx(tx, &models.Actress{JapaneseName: "key"})
	require.Error(t, err)
	_, err = resolveAmbiguousCandidateTx(tx, &models.Actress{JapaneseName: "key"}, "key")
	require.Error(t, err)
