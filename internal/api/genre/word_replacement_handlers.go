...
func listWordReplacements(deps GenreDeps) gin.HandlerFunc {
...
func createWordReplacement(deps GenreDeps, invalidate invalidateCaches) gin.HandlerFunc {
	return func(c *gin.Context) {
...
func updateWordReplacement(deps GenreDeps, invalidate invalidateCaches) gin.HandlerFunc {
	return func(c *gin.Context) {
...
func deleteWordReplacement(deps GenreDeps, invalidate invalidateCaches) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if idStr != "" {
			...
			replacement, err := deps.WordReplacementRepo.FindByID(c.Request.Context(), uint(id))
			if err != nil {
				if database.IsNotFound(err) {
					c.JSON(http.StatusNotFound, contracts.ErrorResponse{Error: "word replacement not found"})
					return
		...
		if original != "" {
			existing, err := deps.WordReplacementRepo.FindByOriginal(c.Request.Context(), original)
			if err != nil {
				if database.IsNotFound(err) {
					c.JSON(http.StatusNotFound, contracts.ErrorResponse{Error: "word replacement not found"})
					return
...
func exportWordReplacements(deps GenreDeps) gin.HandlerFunc {
	return func(c *gin.Context) {
		replacements, err := deps.WordReplacementRepo.List(c.Request.Context())
		if err != nil {
			core.RespondInternalError(c, err)
...
func importWordReplacements(deps GenreDeps, invalidate invalidateCaches) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, item := range req.Replacements {
			...
			existing, err := deps.WordReplacementRepo.FindByOriginal(c.Request.Context(), orig)
			if err != nil && !database.IsNotFound(err) {
				errorsCount++
				continue
			}
