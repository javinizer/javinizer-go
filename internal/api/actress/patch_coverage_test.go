...
func TestSyncCreationUsesWriteProtectedGroup(t *testing.T) {
	...
	for _, tc := range []struct{ method, path string }{
		{http.MethodPost, "/actresses"},
		{http.MethodPut, "/actresses/1"},
		{http.MethodDelete, "/actresses/1"},
		{http.MethodPost, "/actresses/merge"},
		{http.MethodPost, "/actresses/merge/preview"},
		{http.MethodPost, "/actresses/import"},
		{http.MethodPost, "/actresses/sync-jobs"},
		{http.MethodPost, "/actresses/sync-jobs/job-1/cancel"},
	} {
		req := httptest.NewRequest(tc.method, tc.path, strings.NewReader(`{"scope":"missing"}`))
		req.Header.Set("Content-Type", "application/json")
		response := httptest.NewRecorder()
		router.ServeHTTP(response, req)
		require.Equal(t, http.StatusTooManyRequests, response.Code, tc.method+" "+tc.path)
	}
}
...
func TestSyncJobTaskViews(t *testing.T) {
	...
	t.Cleanup(rt.Shutdown)
	now := time.Now().UTC()
	job := &models.ActressSyncJob{ID: "task-view-job", Status: models.ActressSyncJobCompleted, Scope: "missing", CreatedAt: now}
...
func TestSyncHandlersReportUnavailableManager(t *testing.T) {
	...
	for _, request := range requests {
		...
		if request.method == http.MethodGet && (request.path == "/jobs/id" || request.path == "/jobs/id/tasks") {
			expectedStatus = http.StatusNotFound
		}
		assert.Equal(t, expectedStatus, w.Code, "%s %s: %s", request.method, request.path, w.Body.String())
	}
}
