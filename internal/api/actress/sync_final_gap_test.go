...
func TestFinalGap_CandidatesInvalidOffset(t *testing.T) {
	...
	router.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/candidates?offset=-1", nil))
	assert.Equal(t, http.StatusBadRequest, w.Code)
	w2 := httptest.NewRecorder()
	router.ServeHTTP(w2, httptest.NewRequest(http.MethodGet, "/candidates?offset=abc", nil))
	assert.Equal(t, http.StatusBadRequest, w2.Code)
}

func TestFinalGap_ListTasksCountError(t *testing.T) {
	db, err := database.New(&database.Config{Type: "sqlite", DSN: ":memory:"})
	require.NoError(t, err)
	require.NoError(t, db.RunMigrationsOnStartup(context.Background()))
	coreDeps, err := commandutil.NewDependenciesWithOptions(config.DefaultConfig(nil, nil), &commandutil.DependenciesOptions{DB: db, ScraperRegistry: scraperutil.NewScraperRegistry()})
...
func TestFinalGap_CancelGetError(t *testing.T) {
	db, err := database.New(&database.Config{Type: "sqlite", DSN: ":memory:"})
	require.NoError(t, err)
	require.NoError(t, db.RunMigrationsOnStartup(context.Background()))
	coreDeps, err := commandutil.NewDependenciesWithOptions(config.DefaultConfig(nil, nil), &commandutil.DependenciesOptions{DB: db, ScraperRegistry: scraperutil.NewScraperRegistry()})
