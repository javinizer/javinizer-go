...
func TestValidateAndResolverEnrichmentRemainingBranches(t *testing.T) {
	...
	assert.Zero(t, enrichActressesFromResolvers(t.Context(), &models.Movie{Actresses: []models.Actress{complete}}, newTestRegistry(resolver), &Config{ScrapeActress: true}))
	...
	assert.Equal(t, 1, enrichActressesFromResolvers(t.Context(), &models.Movie{Actresses: []models.Actress{{}}}, newTestRegistry(first, second), &Config{ScrapeActress: true}))
