...
// init. Mirrors the downloader's wave-7 rollback restore open
// (internal/downloader/restore_source_nofollow_windows.go): the history
// package's restore path opened backups with a plain open on Windows until
// wave-9, leaving the Lstat gate's TOCTOU unclosed there.
const restoreSourceNoFollow = 0

// init installs the reparse-point restore open as the package-wide source
// opener before any revert restore can run.
func init() {
	restoreOpenReplacementSource = openRestoreSourceWindows
}

// openRestoreSourceWindows opens a journaled restore backup WITHOUT following
// a final-component symlink: FILE_FLAG_OPEN_REPARSE_POINT makes CreateFile
