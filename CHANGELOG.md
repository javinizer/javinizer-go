...
- Add bidirectional favorite-genre shuttle in movie editor (#169)
- Add favorite genre quick-apply palette (#168)
- Show existing NFO values alongside scraped data (#160)

### Fixed

- Sync scraper ProxyEnabled with resolved proxy mode (#167)
- Use CREATE_NO_WINDOW for silent Windows upgrade helper (#166)

## [v1.3.2] - 2026-07-21

### Added

- Group actress min threshold config option (#163)
- Language selector on pre-auth (setup/login) screens (#165)

### Fixed

- Infinite page-refresh loop for Simplified/Traditional Chinese browser locales (#164, #165)

## [v1.3.1] - 2026-07-19

### Added

- Minimal config persistence with user-only scraper overrides and sparse saves (#161)
- Surface localized, typed scrape-error messages (#157)

### Fixed

- Separate title and display_title fields to prevent code doubling (#159)
- Raise request_timeout_seconds default to 180s and keep environment overrides out of config.yaml
...
- Generate 3-digit padded content_id variations for lookup prefixes (fixes ABF-346 and similar titles)

### Changed

- Fix stale doc comment on generateContentIDVariations
