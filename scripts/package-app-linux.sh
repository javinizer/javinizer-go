...
# so we always provide one: use the caller's icon if given, else a 64x64
# solid-color PNG (base64-embedded so it survives the shell literally).
# appimagetool uses CImg internally, which is strict about PNG formatting,
# so the placeholder is generated by a real encoder (zlib) rather than
# hand-crafted bytes. A real icon should be wired in later for polished
# desktop integration.
