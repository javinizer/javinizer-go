...
# generated self-ignored subtree (the paraglide pattern): i18n output is
# REGENERATED by npm/CI, never part of the snapshot — blocking it would
# reject every frontend commit after npm ci; real scratch stays caught
reset; printf '\nexport const y = 2;\n' >> web/frontend/src/app.ts; git add web/frontend/src/app.ts
mkdir -p web/frontend/src/lib/paraglide
printf '*\n' > web/frontend/src/lib/paraglide/.gitignore
printf 'export const m = {};\n' > web/frontend/src/lib/paraglide/messages.js
check_hook "e2e: generated ignored subtree (paraglide) does not block" 0
