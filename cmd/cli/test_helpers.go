...
// withTempConfig temporarily sets the global config for a test.
// Restores the original config after the test function completes.
//
// Usage:
//
//	withTempConfig(t, testCfg, func() {
//	    // Test code that uses global cfg variable
//	})
func withTempConfig(t *testing.T, testCfg *config.Config, fn func()) {
	t.Helper()
...
// withTempConfigFile temporarily sets the global config file path for a test.
// Restores the original path after the test function completes.
//
// Usage:
//
//	configPath, _ := createTestConfig(t)
//	withTempConfigFile(t, configPath, func() {
//	    // Test code that uses global cfgFile variable
//	})
func withTempConfigFile(t *testing.T, path string, fn func()) {
	t.Helper()
